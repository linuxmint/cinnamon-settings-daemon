...
static void
csd_sharing_manager_start_services (CsdSharingManager *manager)
{
        GList *services, *l;
...
static void
csd_sharing_manager_stop_services (CsdSharingManager *manager)
{
        GList *services, *l;
...
static const char *
get_type_for_connection_id (CsdSharingManager *manager,
                            const char        *id)
{
                            ...
                            gpointer    user_data)
{
        ...
        if (manager->priv->sharing_status == CSD_SHARING_STATUS_AVAILABLE)
                csd_sharing_manager_start_services (manager);
        else
                csd_sharing_manager_stop_services (manager);
}
