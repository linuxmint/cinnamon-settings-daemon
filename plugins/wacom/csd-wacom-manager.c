...
#define CSD_WACOM_DBUS_PATH CSD_DBUS_PATH "/Wacom"
#define CSD_WACOM_DBUS_NAME CSD_DBUS_NAME ".Wacom"

#define LEFT_HANDED_KEY     "left-handed"

static const gchar introspection_xml[] =
"<node name='/org/cinnamon/SettingsDaemon/Wacom'>"
"  <interface name='org.cinnamon.SettingsDaemon.Wacom'>"
...
static gboolean set_led (const gchar  *device_path,
                         ...
                         guint         index,
                         GError      **error);
static gboolean is_opaque_tablet (CsdWacomManager *manager,
                                  GdkDevice       *device);
...
static void
migrate_tablet_settings (CsdWacomManager *manager,
                         GdkDevice       *device)
{
        CsdSettingsMigrateEntry tablet_settings[] = {
        ...
        old_path = g_strdup_printf ("/org/cinnamon/settings-daemon/peripherals/wacom/%s-usb:%s:%s/",
                                    manager->machine_id, vendor, product);
        new_path = g_strdup_printf ("/org/cinnamon/desktop/peripherals/tablets/%s:%s/",
                                    vendor, product);

        csd_settings_migrate_check ("org.cinnamon.settings-daemon.peripherals.wacom.deprecated",
                                    old_path,
                                    "org.cinnamon.desktop.peripherals.tablet",
                                    new_path,
        ...
        if (is_opaque_tablet (manager, device)) {
                CsdSettingsMigrateEntry display_setting[] = {
                        { "display", "output", NULL },
                };

                csd_settings_migrate_check ("org.cinnamon.desktop.peripherals.tablet.deprecated",
                                            new_path,
                                            "org.cinnamon.desktop.peripherals.tablet",
                                            new_path,
...
static gboolean
is_opaque_tablet (CsdWacomManager *manager,
                  GdkDevice       *device)
{
...
static GdkDevice *
lookup_device_by_path (CsdWacomManager *manager,
                       const gchar     *path)
{
        GList *devices, *l;
...
device_get_settings (GdkDevice *device)
{
        ...
        settings = g_settings_new_with_path ("org.cinnamon.desktop.peripherals.tablet",
                                             path);
        g_free (path);
...
handle_method_call (GDBusConnection       *connection,
                    ...
                    gpointer               data)
{
    CsdWacomManager *self = CSD_WACOM_MANAGER (data);
        GError *error = NULL;
        GdkDevice *device;
...
static void
add_devices (CsdWacomManager     *manager,
...
static void
register_manager (CsdWacomManager *manager)
{
...
get_machine_id (void)
{
        ...
        no_per_machine_file = g_build_filename (g_get_user_config_dir (), "cinnamon-settings-daemon", "no-per-machine-config", NULL);
                         ...
                         GError         **error)
{
        ...
        g_source_set_name_by_id (manager->start_idle_id, "[cinnamon-settings-daemon] csd_wacom_manager_idle_cb");
        cinnamon_settings_profile_end (NULL);
